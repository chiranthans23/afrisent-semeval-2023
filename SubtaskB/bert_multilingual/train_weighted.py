...
from utils import loss_fn, score, multi_acc, plot_met_curves
from dataset import get_train_val_loaders2_weighted, get_lang_dataset
import gc
import numpy as np
...
def train():
    ...
    train_data, val_data, class_weights_all = get_lang_dataset()
    ...
    criterion = loss_fn()   
    dataloaders_dict = get_train_val_loaders2_weighted(train_data, val_data, class_weights_all, config['batch_size'])
    ...
    for epoch_num in range(config['num_epochs']):
        ...
        torch.save(model.state_dict(), f"./models/{config['model_name']}_epoch{epoch_num+1}.pth")

    plot_met_curves(f1_met, loss_met)        
    return
